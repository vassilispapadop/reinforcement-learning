...
Figure 2 shows the number of iterations required from the algorithm to converge for different $\gamma$ values. 
	...
	Having calculated the optimal values $V^{*}$ for the 3x4 grid world, we will use them as heuristic to find the sortest path from position (2,0) to terminal state with higher reward (0,3). The output of the algorithm is shown in table below. We notice for $\gamma=0.2$ path could not be find.
	
		\begin{center}
		\begin{tabular}{ l l l l l l l }
			$\gamma=0.9$ & (2,0)  & (1,0) & (0,0)  & (0,1) & (0,2) & (0,3) \\ 
			$\gamma=0.6$  & (2,0)  & (1,0) & (0,0)  & (0,1) & (0,2) & (0,3) \\ 
			$\gamma=0.2$  & (2,0)  & - & -  & - & - & - 
		\end{tabular}
